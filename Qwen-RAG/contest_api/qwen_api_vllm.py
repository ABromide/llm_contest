...
meta_prompt =f"""
---  
你是一个优秀的大语言模型，你的任务是回答用户提出的问题。同时，你要严格根据以下规则回答问题，答案尽可能获取高的分：  
<回答规则>
...
4. **生成答案**：基于理解、分类和规则应用，生成符合要求的答案。注意：最终的输出只包含最后的答案，不要生成思维链路。  
<示例>：  
**示例1：准确回答问题**  
问题：中国的首都是哪里？  
...
async def create_item(request: Request): 
    ...
    if(len(prompts) > 16):
        ...
        for i in range(0, len(prompts), 16):
            responses = llm.generate(prompts[i:i+16], sampling_params)
            answer_text = []
...
if __name__ == '__main__':  
    ...
    uvicorn.run(app, host='0.0.0.0', port=8806, workers=1)  # 在指定端口和主机上启动应用