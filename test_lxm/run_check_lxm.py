...
URL = "http://127.0.0.1:8802"  # 修正了URL中的错误字符  
...
def get_check_completion(prompt):  
    headers = {'Content-Type': 'application/json'}  
...
def process_text_input(text_input):  
    ...
    value = get_check_completion(api_output)
...
def main(to_pred_dir, result_save_path):
    ...
    csv_path = os.path.join(to_pred_dir, "forbidden_question_set.csv")
    with open(csv_path, "r", encoding="utf-8") as f:
        reader = csv.reader(f)
        next(reader)  # 跳过表头
        lines = [row[3] for row in reader]  # 提取 question 列
    ...
    if num_process > 1:  # 多进程模式
        ...
        with open(result_save_path, "w") as fw:
            for line in lines:
                ...
                fw.write("%s\t%s\t%s\n" % (output["input"], cleaned_output, output["value"]))
                fw.flush()
...
#     with open(result_save_path, "w", newline='', encoding='utf-8') as fw:
#         writer = csv.writer(fw)
#         # 写入表头
#         writer.writerow(['num', 'question', 'response', 'value'])
...
#                     value = output["value"]
#                     # 写入行
#                     writer.writerow([index, question, response, value])
#                     fw.flush()
...
#                 output = process_text_input(line)
#                 response = output["output"].replace("\\n", "").replace("\\t", "").replace("\\r", "").replace("\n", "").replace("\t", "").replace("\r", "")
#                 value = output["value"]

#                 # 写入行
#                 writer.writerow([index, line, response, value])
#                 fw.flush()
